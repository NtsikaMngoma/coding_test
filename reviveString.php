...
// Our method for generating our substrings
function generateSubStrings($string) : array {
    $substrings = array();

    // We are getting the desired string length within the loop
    for ($start = 0; $start < strlen($string) - 1; $start++)
        for ($length = 1; $length < strlen($string) - $start+1; $length++)
            $substrings = substr($string, $start, $length);
    // Returning the specified substrings
    return $substrings;
}
...
function reviveString(string $string, int $k): string {
    ...
    //Function to get all the unique substrings
    $sub_strings = generateSubStrings($string);

    // Initialize sub_string to the array_unique PHP function to keep all unique strings and remove duplicates from the array
    $unique_sub_strings = array_unique($sub_strings);

    // Sort the above variable, initialized as an array, based on the lexigraphic requirements
    natcasesort($unique_sub_strings);

    // Concatenating the substrings and retrieving them from the array elements
    $concatenate = implode('', $unique_sub_strings);

    // Returning the parameter defined in this method and negating by -1 within the array
    return $concatenate[$k - 1];
}
